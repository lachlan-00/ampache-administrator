...
if [ $BRANCH = "master" ] || [ $BRANCH = "all" ]; then
  ...
  cd $AMPACHEDIR/docker/ampache-docker/ && git checkout master && git reset --hard origin/master && git pull && docker buildx build --no-cache --platform linux/amd64,linux/arm64,linux/arm/v7 --build-arg VERSION=${RELEASEVERSION} -t ampache/ampache:6 -t ampache/ampache:${RELEASEVERSION} -t ampache/ampache:latest --push . &
fi
...
if [ $BRANCH = "nosql" ] || [ $BRANCH = "all" ]; then
  ...
  cd $AMPACHEDIR/docker/ampache-docker-nosql/ && git checkout nosql && git reset --hard origin/nosql && git pull && docker buildx build --no-cache --platform linux/amd64,linux/arm64,linux/arm/v7 --build-arg VERSION=${RELEASEVERSION} -t ampache/ampache:nosql6 -t ampache/ampache:nosql${RELEASEVERSION} -t ampache/ampache:nosql --push . &
fi
